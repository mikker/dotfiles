...
const rootReadTool = createReadTool(process.cwd());

type ToolTheme = Parameters<
  NonNullable<ExtensionAPI["registerTool"]>
>[0]["renderCall"] extends (args: any, theme: infer T, context: any) => any
  ? T
  : never;
...
export default function readSummaryExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    label: "read",
    description:
      "Read file contents. UI rendering shows a compact summary instead of file content previews.",
    parameters: rootReadTool.parameters,
