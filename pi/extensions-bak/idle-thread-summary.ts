...
function renderSummary(summary: string, width: number): string[] {
  const maxInnerWidth = Math.max(8, Math.min(76, width - 4));
  const items = summary
    .split(/\n+/)
  ...
  const contentLines = bullets.flatMap((item) => {
    const wrapped = wrapTextWithAnsi(item, Math.max(1, maxInnerWidth - 2));
    return wrapped.map((line, index) => `${index === 0 ? "•" : " "} ${line}`);
  });
  const innerWidth = Math.min(
    maxInnerWidth,
    Math.max(8, ...contentLines.map((line) => visibleWidth(line))),
  );
  ...
  return [
    ...
    ...contentLines.map((line) => {
      ...
      return `${DIM}│${RESET} ${ACCENT}${clipped}${padding}${RESET} ${DIM}│${RESET}`;
    }),
    `${DIM}└${border}┘${RESET}`,
  ];
}
...
export default function idleThreadSummaryExtension(pi: ExtensionAPI) {
  ...
  function schedule(ctx: ExtensionContext) {
    clear(ctx);
    if (ctx.mode !== "tui" || ctx.hasPendingMessages()) return;
    ...
    timer = setTimeout(async () => {
      ...
      controller = request;
      try {
        const summary = await summarizeThread(ctx, request.signal);
        if (generation !== myGeneration || !summary) return;
        ctx.ui.setWidget(WIDGET_ID, () => ({
          render: (width) => renderSummary(summary, width),
          invalidate() {},
        }));
      } catch (error) {
        if (!request.signal.aborted && generation === myGeneration) {
