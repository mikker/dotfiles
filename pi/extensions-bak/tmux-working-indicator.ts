...
const LOCK = "pi-working-indicator";
const GLYPH = "󱚣";
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async () => updateState(false));
  pi.on("agent_start", async () => updateState(true));
  pi.on("agent_settled", async () => updateState(false));
  pi.on("session_shutdown", async () => updateState(false));
}

async function updateState(active: boolean) {
  await tmux(["wait-for", "-L", LOCK]);
  try {
    const pids = (await livePids()).filter((pid) => pid !== PID);
    if (active) pids.push(PID);
    ...
    // Keep the conditional wrapper installed globally; the window-scoped option
    // decides visibility and avoids one window hiding another window's indicator.
    await setGlyph(true);
  } finally {
    await tmux(["wait-for", "-U", LOCK]);
  }
}
...
async function setGlyph(show: boolean) {
  for (const option of OPTIONS) {
    ...
    await tmux([
      ...
      "-gq",
      option,
      show ? addGlyph(format) : removeGlyph(format),
    ]);
  }
}
