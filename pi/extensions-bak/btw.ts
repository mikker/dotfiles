...
class BtwOverlay extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    const dialogWidth = Math.max(12, Math.min(width, Math.floor(width * 0.9)));
    const innerWidth = Math.max(10, dialogWidth - 2);
...
export default function (pi: ExtensionAPI) {
  ...
  function getModelKey(ctx: ExtensionContext): string {
    ...
    return model
      ? `${model.provider}/${model.id}/${pi.getThinkingLevel()}`
      : "none";
  }
  ...
  async function closeOverlayFlow(
    ...
    if (sideBusy) {
      await activeSideSession?.session.abort();
      sideBusy = false;
      pendingQuestion = null;
      pendingAnswer = "";
      pendingToolCalls = [];
      setOverlayStatus("Request cancelled.");
    }
    dismissOverlay();
    if (!ctx.hasUI) {
      return;
  ...
  pi.registerCommand("btw", {
    ...
    handler: async (args, ctx) => {
      if (ctx.mode !== "tui") {
        ctx.ui.notify("btw is only available in the interactive TUI", "error");
        return;
      }
