...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `/Users/mikker/Library/Application Support/tldraw/server.json`.
...
```bash
PORT=$(jq -r .port '/Users/mikker/Library/Application Support/tldraw/server.json'); TOKEN=$(jq -r .token '/Users/mikker/Library/Application Support/tldraw/server.json')
